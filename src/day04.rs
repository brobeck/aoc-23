...
fn solve_task_2() {
    let input = fs::read_to_string("input/input_4.txt").unwrap();
    let mut card_number_array: [usize; 100] = [0; 100];
    let mut number_of_that_card: [usize; 189] = [1; 189];
    let mut set_number_array: [usize; 10] = [0; 10];

    for (enumeration, line) in input.lines().enumerate() {
        let (card_numbers, guess_numbers) = line.split_once('|').unwrap();
        ...
        let to_index: usize = cmp::min(189, enumeration + 1 + correct_guesses);
        for i in enumeration + 1..to_index {
            number_of_that_card[i] += number_of_that_card[enumeration];
        }
    }

    let ans: usize = number_of_that_card.iter().sum();
